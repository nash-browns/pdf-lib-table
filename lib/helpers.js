import { measureText, segmentText, wrapText } from './textMeasurement';
...
export function getCellPadding(options = {}, context = 'cell') {
    ...
    if(context === 'header') {
        horizontalLine = (options.headerDividedX ?? true) ? (options.headerDividedXThickness ?? 1) : 0;
        verticalLine = (options.headerDividedY ?? true) ? (options.headerDividedYThickness ?? 1) : 0;
    } else if(context === 'subheading') {
        horizontalLine = options.subHeadingDividedX ? (options.subHeadingDividedXThickness ?? 1) : 0;
        verticalLine = options.dividedY ? (options.dividedYThickness ?? 1) : 0; //subheading cells draw the table-level vertical dividers
    } else {
        horizontalLine = options.dividedX ? (options.dividedXThickness ?? 1) : 0;
        verticalLine = options.dividedY ? (options.dividedYThickness ?? 1) : 0;
    }
