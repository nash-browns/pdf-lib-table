...
//a user-specified column width is non-negotiable: min = max = the requested
//width, and the fixedWidth flag tells the later passes to leave it alone
function fixedColumnDimensions(width, tableMaxWidth) {
    return {
        columnMinWidth: width,
        maxColumnWidth: width,
        intrinsicPercentageWidth: updateIntrinsicPercentageWidth(width, tableMaxWidth),
        fixedWidth: width,
    };
}
...
export function columnWidthNoWrap(columns, options) {
    ...
    columns.forEach((col) => {
        if(col.width > 0) {
            columnDimensions = {...columnDimensions, [col.columnId]: fixedColumnDimensions(col.width, tableMaxWidth)};
            return;
        }
