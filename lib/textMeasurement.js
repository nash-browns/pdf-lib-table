...
//single-line truncation: returns the text unchanged if it fits maxWidth,
//otherwise the longest prefix that fits with a trailing ellipsis
export function truncateText(font, size, maxWidth, text) {
    const str = text == null ? '' : String(text);
    if (measureText(font, size, str) <= maxWidth) return str;
    ...
    while (low < high) {
        ...
        if (measureText(font, size, str.slice(0, mid).trimEnd() + ELLIPSIS) <= maxWidth) low = mid;
        else high = mid - 1;
    }
    return str.slice(0, low).trimEnd() + ELLIPSIS;
}
